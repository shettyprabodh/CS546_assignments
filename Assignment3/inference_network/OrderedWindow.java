...
public class OrderedWindow extends WindowNode{
  ...
  public OrderedWindow(ArrayList<TermNode> children, int window_size, InvertedIndex index){
    super(children, window_size, index);
    this.removeUnsortedPostingList();

    System.out.println("(Inside OrderedWindow)Final postings list:- ");
  ...
  public void removeUnsortedPostingList(){
    ...
    for(int i=0; i<this.final_postings_list.size(); i++){
      ...
      for(int j=0; j<all_positions.size(); j++){
        if(OrderedWindow.isSorted(all_positions.get(j))){
